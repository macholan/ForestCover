...
``` {r forestcoverbarplot, echo = FALSE, display = TRUE, fig.cap="Forest Cover Type Frequency", fig.width = 2.5, fig.align = "center"}
...
The scatterplots in Figure 5 help us examine these highly correlated variables more closely. From the plots, we observe the following: 
...
A dot plot of forest cover type by wilderness area in Figure 6 shows the following trends:  
...
A dot plot of forest cover type by soil type in Figure 7 also shows variability in the proportion of tree types from soil to soil. For example, some soil types show a very high proportion of Lodgepole Pines (e.g. Soil Type 7) whereas others show a very low proportion of Lodgepole Pines (e.g. Soil Type 37). The distinctive make-up of trees by soil type suggests that this may be a good predictive factor for our models. 
...
Other variables that could be important to defining models are shown in the variable importance plot in Figure 8.

```{r rpartmodel, eval = FALSE, echo=FALSE, include = FALSE}
treefit <- rpart(Cover_Type ~ ., method = "class", data = covtype.training[c(1:55)])
treefit <- treefit
```

```{r rpartsummary, eval = FALSE, echo=FALSE, include = FALSE}
...
```{r rpartplotvarimpt, eval = FALSE, echo = FALSE, display = TRUE, fig.height = 2.5, fig.width = 4, fig.cap = 'Naive Classification Tree Variable Importance Plot', fig.align="center"}
...
```{r rpartplot, eval = FALSE, echo = FALSE, display = FALSE, fig.height = 4.3, fig.cap= "Naive Classification Tree", fig.aligh = "center"}
rpart.plot(treefit, uniform=TRUE, extra=104, main="", cex = 0.7)
```

### Random Forest Model

According to the importance plot for a random forest model, the elevation variable has the higest importance, followed by soil type, the horizontal distance to roadways, and the horizontal distance to wildfire points. Figure 9 shows the ranking and relative importance of the variables selected.

``` {r randomforest, eval = FALSE, echo = FALSE, display = FALSE}
...
``` {r richard, echo = FALSE, eval = FALSE}
# from Richard
