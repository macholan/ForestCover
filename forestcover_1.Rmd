...
``` {r forestcoverbarplot, echo = FALSE, display = TRUE, fig.cap="Forest Cover Type Frequency", fig.align = "center"}
...
``` {r densityplots, echo=FALSE, display = TRUE, fig.cap = "Density plots of numeric variables", fig.aligh = "center"}
# create density plot
require(captioner)
p=list()
for(i in 1:10){
    p[[i]] <- ggplot(covtype, aes_string(x=covtype[,i])) + 
      geom_density() + 
      #theme_adMobile() + 
...
To understand the relationships between the numeric predictor variables, we can examine the plot in Figure X for important relationships.
...
The scatterplots in Figure X help us examine these highly correlated variables more closely. From the plots, we observe the following: 
* The hillshade at noon and 3pm creates an elipsoid pattern. 
...
* Hillshade at 3pm has a sigmoid relationship with Aspect. 
* Aspect and Hillshade at 9am have a more defined sigmoid relationship. 

``` {r scatterplots, echo=FALSE, display = TRUE, fig.caption = "Scatterplots of Highly Correlated Numeric Variables, fig.align = "center"}
...
A dot plot of forest cover type by wilderness area shows the following trends:
...
A dot plot of forest cover type by soil type also shows variability in the proportion of tree types from soil to soil. For example, some soil types show a very high proportion of Lodgepole Pines (e.g. Soil Type 7) whereas others show a very low proportion of Lodgepole Pines (e.g. Soil Type 37). The distinctive make-up of trees by soil type suggests that this may be a good predictive factor for our models. 

```{r soildotplot, echo=FALSE, display = TRUE, fig.caption="Proportion of Forest Cover by Soil Type", fig.align="center"}
...
Other variables that could be important to defining models are shown in the variable importance plot in Figure X.
...
```{r rpartplotvarimpt, echo = FALSE, display = TRUE, fig.height = 2.5, fig.width = 4, fig.cap = 'Naive Classification Tree Variable Importance Plot', fig.align="center"}
barplot(treefit$variable.importance[order(treefit$variable.importance)], 
        cex.names = 0.5, horiz = TRUE, cex.axis = 0.5, las=1)
```

A plot of the decision tree shows that this model has significant limitations and ignores the rarer tree types in partitioning the data. With an accuracy of only 7%, we learn that a much more sophisticated model will be required to predict cover type.

```{r rpartplot, echo = FALSE, display = FALSE, fig.height = 4.3, fig.cap= "Naive Classification Tree", fig.aligh = "center"}
rpart.plot(treefit, uniform=TRUE, extra=104, main="", cex = 0.7)
```

### Random Forest Model

According to the importance plot for a random forest model, the elevation variable has the higest importance, followed by soil type, the horizontal distance to roadways, and the horizontal distance to wildfire points.

``` {r randomforest, echo = FALSE, display = FALSE}
...
``` {r randomforestimpt, echo = FALSE, display = TRUE, fig.cap = 'Random Forest Variable Importance Plot', fig.align="center"}
...
According to the boruta algorithm for feature selection, elevation has the higest importance, but the second highest is different from the random forest importance index. 
...
``` {r borutaplot, echo = FALSE, display = FALSE, fig.cap = 'Boruta Algorithm Variable Importance Plot', fig.align="center"}
# Boruta Model
plot(bor.results)
names(bor.results)
